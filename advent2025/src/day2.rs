...
fn find_password(path: &str) -> u128 {
    ...
    for range in ranges {
        ...
        println!(
            "Start {}, End {}, Invalid IDs: {:?}",
            range.start, range.end, invalid_ids
        );
...
fn find_new_password(path: &str) -> u128 {
    ...
    for range in ranges {
        ...
        for repeat_count in 2..=max_repeat_count {
            let invalid_ids_for_repeat_count =
                find_invalid_ids(range.start, range.end, repeat_count);
            if !invalid_ids_for_repeat_count.is_empty() {
                print!(
                    ", Invalid IDs (repeat {}): {:?}",
                    repeat_count, invalid_ids_for_repeat_count
                );
            }
...
pub fn find_first_invalid_id_sequence(range_start: u128, repeat_count: usize) -> u128 {
    ...
    if duplicate_block(first_sequence, repeat_count) < range_start {
...
fn check_part2_example(#[case] path: &str, #[case] expected_password: u128) {
    // Act
